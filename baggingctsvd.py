...
cv_add = cv_train_2[abs(cv_train_2['newrating']-cv_train_2['Rating'])>2]
#drop newrating
cv_add[['UserID','ProfileID','Rating']]
#do same algorithm to find prediction rule on only those samples that were poorly predicted on
user_profile_matrix1 = pd.concat([cv_add,cv_test]).pivot('UserID','ProfileID','Rating')
...
while iteration1<10:
    iteration1 += 1
    svd1 = TruncatedSVD(n_components=40,random_state=42)
    svd1.fit(mz1)
